...
function SuccessStoriesPageContent() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // Determine initial selected story
      if (data.length > 0) {
        if (storySlug) {
          const matched = data.find(s => s.slug === storySlug || s.id === storySlug);
          if (matched) {
            setSelectedStory(matched);
            return;
          }
        }
        // Fallback to featured story or the first one
        const featured = data.find(s => s.isFeatured);
        setSelectedStory(featured || data[0]);
      }
    }
    loadData();
  }, [storySlug]);
  ...
  // Steps for currently selected story
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 flex flex-col font-sans">
              ...
              return (
                <div
                  ...
                  className={`group bg-white rounded-2xl overflow-hidden border cursor-pointer flex flex-col justify-between hover:shadow-lg transition-all duration-300 hover:-translate-y-1 ${
                    isSelected 
                      ? 'border-[#5870F7] ring-2 ring-[#5870F7]/10' 
      ...
      {/* Selected Case Study Details Section */}
      {selectedStory && (
        <div 
          ref={detailsRef}
          id="details-section"
          className="bg-white py-16 sm:py-24 border-t border-gray-200/60 scroll-mt-20"
        >
